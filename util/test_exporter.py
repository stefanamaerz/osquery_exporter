...
class ProcessSampler:
    ...
    def start(self, duration: int, interval: float = 0.1) -> None:
        # Clear the stop event so the sampler can be reused across phases
        # (e.g. baseline then load). Without this, a previously-stopped
        # sampler's _sample loop exits immediately and collects nothing.
        self._stop.clear()
    ...
    def _sample(self, duration: int, interval: float) -> None:
        ...
        while time.time() - start < duration and not self._stop.is_set():
            ...
            elif first and found:
                # keep first sample for RSS even though CPU delta is meaningless
                self.samples.append((elapsed, 0.0, total_rss_kb / 1024.0))
...
def sample_baseline(samplers: list[ProcessSampler], duration: int) -> None:
    ...
    print("\n==== Process metrics ====")
    for sampler in samplers:
        print(f"{sampler.name}: collected {len(sampler.samples)} samples")
...
class RateLimiter:
    ...
    Limits the aggregate rate of acquire() calls across all threads to
    `rate` per second. When rate <= 0, acquire() is a no-op (unthrottled).
    """
    ...
    def acquire(self) -> None:
        ...
        with self._lock:
            now = time.monotonic()
            # Never let the schedule fall behind "now" after an idle gap, so a
            # burst isn't issued to "catch up" after a quiet period.
...
def sample_load(
    ...
    If rps > 0, the aggregate request rate across all clients is capped at rps
    (open-loop); otherwise clients issue requests back-to-back (closed-loop).
    """
    mode = f"{rps:g} rps cap" if rps > 0 else "unthrottled (max throughput)"
    print(f"\n=== Load test: {concurrency} clients for {duration}s, {mode} ===")
    ...
    errors: list[str] = []
    threads = [
        threading.Thread(target=_request_loop, args=(base_url, stop, latencies, errors, limiter))
...
def _request_loop(base_url: str, stop: threading.Event, latencies: list[float],
                  errors: list[str], limiter: RateLimiter) -> None:
    while not stop.is_set():
        # Block until the rate limiter grants a slot. Throttling wait-time is
        # NOT counted as latency: the clock starts only after a slot is granted.
        limiter.acquire()
        if stop.is_set():
            break
        t0 = time.time()
...
def main() -> int:
    ...
    parser.add_argument("--load-rps", type=float, default=0.0,
                        help="Cap aggregate requests/sec across all clients (open-loop). "
                             "Default 0 = unthrottled max-throughput (closed-loop). "
                             "Example: --load-rps 0.2 simulates one scrape every 5s")
    ...
    if not args.correctness_only:
        ...
        _, errors = sample_load(args.url, args.load_duration, args.load_concurrency, samplers, args.load_rps)
        if errors > 0:
            ok = False
