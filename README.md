...
### Shared named queries

If several metrics are derived from the same osquery table, you can define the SQL once under the top-level `queries` key and reference it from each metric with `queryref`. This keeps the YAML shorter and lets the exporter execute the query once per scrape, sharing the result among all metrics that reference it.
...
`queryref` and `query` are mutually exclusive on a metric. `query` continues to work for inline SQL and backward compatibility.
